...
from api.handler import get_albums, get_album
import json
...
def index():
    c = Album(request.args.get('name'), str(
        request.args.get('private')) == "True")
    c.add()
    return c.as_dict()
...
def update_album(name: str):
    c = Album.query.filter_by(name=name).first()
    c.update(request.args.get('name'), request.args.get(
        'date'), request.args.get('private') == "True")
    return c.as_dict()
